...
deviatiiarov_path <- here(project_path, "public_transcriptomes", "data", "deviatiiarov_2023_nat_cv_research")
# dir(deviatiiarov_path)
...
library(ggrepel) 
source_data_fig3 <- readxl::read_xlsx(here(deviatiiarov_path, "44161_2022_182_MOESM7_ESM.xlsx"), 
                                      skip = 2,
...
source_data_fig1_explanations <- readxl::read_xlsx(
  here(deviatiiarov_path, "44161_2022_182_MOESM6_ESM.xlsx"),
  skip = 0,
  n_max = 54
) %>%
  s_names
...
source_data_fig3_a_v <- readxl::read_xlsx(here(deviatiiarov_path, "44161_2022_182_MOESM7_ESM.xlsx"), 
                                          skip = 2,
...
```{r PANCR region beta 7 4, fig.width = 7, fig.height = 4}
patch & scale_y_continuous(limits = c(0, 0.35)) & selected_theme + theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.5), "cm"))
```

```{r PANCR region beta 9 5, fig.width = 9, fig.height = 5}
patch & scale_y_continuous(limits = c(0, 0.35)) & selected_theme + theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.5), "cm"))
```

```{r PANCR region beta 10 6, fig.width = 10, fig.height = 6}
patch & scale_y_continuous(limits = c(0, 0.35)) & selected_theme + theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.5), "cm"))
```
...
```{r PANCR region 8 4, fig.width = 8, fig.height = 4, eval = FALSE}
p_region
```

```{r PANCR region 8 5, fig.width = 8, fig.height = 5, eval = FALSE}
p_region
```
...
p_region <- 
  ...
  p2 + 
  # p3 +
  plot_layout(ncol = 1, axes = "collect", heights = c(6, 2)) &
  scale_x_continuous(
    labels = comma, 
    # breaks = c(110600000, 110620000,  110630000,110640000, 
    #            PANCR_DMRs$seg_start, PANCR_DMRs$seg_end),
...
```{r PANCR region 8 6 (simple), fig.width = 8, fig.height = 6}
p_region & 
  theme(
...
```{r PANCR region 8 4 (simple), fig.width = 8, fig.height = 4}
p_region & 
  theme(
    axis.text.x = element_text(size = 15),
    legend.title = element_text(size = 15),
...
```{r PANCR region 8 5 (simple), fig.width = 8, fig.height = 5}
p_region & 
  theme(
    axis.text.x = element_text(size = 15),
    legend.title = element_text(size = 15),
